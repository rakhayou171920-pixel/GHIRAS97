      ...
      <div className="container mx-auto px-4 -mt-10 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white rounded-xl p-4 text-center shadow-lg">
            ...
            <div className="text-sm text-gray-500">الترتيب من {totalStudents}</div>
          </div>
          <div className="bg-white rounded-xl p-4 text-center shadow-lg">
        ...
        {tasks.length > 0 && (
          <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div className="p-4 space-y-3">
              {tasks.map(task => (
                <div key={task.id} className={`p-3 rounded-lg border ${task.claimed_by === studentId ? "border-yellow-400 bg-yellow-50" : "border-blue-200 bg-blue-50"}`}>
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    {task.claimed_by === studentId ? (
                      ...
                      <button onClick={() => claimTask(task.id)} className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-xs font-bold" data-testid={`claim-task-${task.id}`}>
                        احجز المهمة
                      </button>
                    ) : null}
                  </div>
            ...
            <div className="divide-y max-h-64 overflow-y-auto">
              {pointsLog.map((log, i) => (
                <div key={i} className="flex items-center justify-between p-3">
                  <div>
                    <p className="text-sm font-semibold text-gray-800">{log.reason}</p>
                    <p className="text-xs text-gray-400">{formatDate(log.created_at)}</p>
                  </div>
                  <span className={`px-2 py-0.5 rounded-full text-sm font-bold ${log.points > 0 ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
